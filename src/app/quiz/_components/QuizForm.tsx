...
export function QuizForm({ mode, initialValues, onSubmit, isSubmitting }: QuizFormProps) {
  ...
  const [instrumentId, setInstrumentId] = useState(initialValues?.instrumentId ?? "");
  ...
  const createComposer = api.composer.create.useMutation({ 
    onSuccess: async () => { await refetchComposers(); }, 
    onError: (error) => { showError(error.message || "Failed to create composer"); } 
  });
  const createInstrument = api.instrument.create.useMutation({ 
    onSuccess: async () => { await refetchInstruments(); }, 
    onError: (error) => { showError(error.message || "Failed to create instrument"); } 
  });
  const createArtist = api.artist.create.useMutation({ 
    onSuccess: async () => { await refetchArtists(); }, 
    onError: (error) => { showError(error.message || "Failed to create artist"); } 
  });

  const updateSlice = (index: number, field: keyof QuizSliceInput, value: string | number) => {
  ...
  const handleSubmit = () => {
    ...
    onSubmit({ composerId, instrumentId, pieceName: pieceName.trim(), duration, slices });
  };

  return (
    <div className="space-y-8">
      <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)]/60 p-8">
        <h2 className="font-body mb-6 text-2xl font-semibold text-[var(--color-text-primary)]">Quiz Setup</h2>
        <div className="space-y-6">
          <div>
            <label className="font-body-semibold block text-sm font-medium text-[var(--color-text-secondary)] mb-2">
          ...
          <div>
            <label className="font-body-semibold block text-sm font-medium text-[var(--color-text-secondary)] mb-2">
              Clip Duration (seconds) <span className="text-[var(--color-error)]">*</span>
            </label>
            <input 
              type="number" 
              min={5} 
            ...
            />
            <p className="font-body-medium mt-1 text-sm text-[var(--color-text-muted)]">Between 5 and 120 seconds</p>
          </div>
        </div>
      </div>

      <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)]/60 p-8">
        <h2 className="font-body mb-6 text-2xl font-semibold text-[var(--color-text-primary)]">Performances (3 required)</h2>
        <div className="space-y-6">
          {slices.map((slice, idx) => (
            <div key={idx} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-input)]/80 p-6">
              <div className="mb-4 flex items-center gap-3">
                ...
                <span className="font-body-semibold font-medium text-[var(--color-text-primary)]">Performance {idx + 1}</span>
              </div>
              <div className="space-y-4">
                ...
                <div>
                  <label className="font-body-semibold block text-sm font-medium text-[var(--color-text-secondary)] mb-2">
                    YouTube URL <span className="text-[var(--color-error)]">*</span>
                  </label>
                  <input 
                    type="url" 
                ...
                <div>
                  ...
                  <p className="font-body mt-2 text-xs text-[var(--color-text-muted)]">Adjust with buttons or type mm:ss</p>
                </div>
                {isValidYouTubeUrl(slice.youtubeUrl) && (
                  <YouTubeClipPicker youtubeUrl={slice.youtubeUrl} startTime={slice.startTime} duration={duration} />
                )}
              </div>
            </div>
          ))}
      ...
      <div className="flex justify-end">
        ...
        >
          {isSubmitting ? (mode === "create" ? "Creating..." : "Updating...") : (mode === "create" ? "Create Quiz" : "Update Quiz")}
        </button>
      </div>
    </div>
  );

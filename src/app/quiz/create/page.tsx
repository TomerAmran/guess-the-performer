      ...
      <main className="min-h-screen bg-[var(--color-bg-primary)] transition-colors duration-300">
        <div className="container mx-auto max-w-4xl px-4 py-12">
          <div className="rounded-2xl border-2 border-[var(--color-success)]/50 bg-[var(--color-success)]/10 p-8 text-center">
            ...
            <h2 className="mb-4 text-3xl font-bold text-[var(--color-success)]" style={{ fontFamily: 'var(--font-body), serif' }}>Quiz Created!</h2>
            <p className="mb-8 text-[var(--color-text-secondary)]" style={{ fontFamily: 'var(--font-body), serif', fontWeight: 500 }}>Your quiz has been saved and is ready to be played.</p>
            <div className="flex justify-center gap-4">
              <button onClick={handleReset} className="rounded-lg border-2 border-[var(--color-border)] px-6 py-3 font-medium text-[var(--color-text-secondary)] transition-all hover:bg-[var(--color-border)]" style={{ fontFamily: 'var(--font-body), serif', fontWeight: 600 }}>Create Another</button>
              <Link href="/" className="rounded-lg bg-[var(--color-accent-gold)] px-6 py-3 font-semibold text-[var(--color-bg-primary)] transition-all hover:bg-[var(--color-accent-gold-hover)]" style={{ fontFamily: 'var(--font-body), serif', fontWeight: 600 }}>Back to Home</Link>
            </div>
          </div>
        </div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-[var(--color-bg-primary)] transition-colors duration-300">
      <div className="container mx-auto max-w-4xl px-4 py-12">
        ...
        <h1 className="mb-2 text-4xl font-bold tracking-tight text-[var(--color-text-primary)]" style={{ fontFamily: 'var(--font-body), serif' }}>Create a Quiz</h1>
        <p className="mb-8 text-lg text-[var(--color-text-muted)]" style={{ fontFamily: 'var(--font-body), serif', fontWeight: 500 }}>Build a quiz where players match performances to artists</p>
        ...
        <div className="space-y-8">
          ...
          <div className="flex justify-end">
            <button onClick={handleSubmit} disabled={!isFormValid() || createQuiz.isPending} className="rounded-lg bg-[var(--color-accent-gold)] px-8 py-3 font-semibold text-[var(--color-bg-primary)] transition-all hover:bg-[var(--color-accent-gold-hover)] disabled:cursor-not-allowed disabled:opacity-40" style={{ fontFamily: 'var(--font-body), serif', fontWeight: 600 }}>{createQuiz.isPending ? "Creating..." : "Create Quiz"}</button>
          </div>
        </div>
      </div>
    </main>
